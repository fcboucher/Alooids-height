...
sp.phylo[-which(sp.phylo%in%sp.SANBI)] # 66 species in phylo with no coordinate data
sp.SANBI[-which(sp.SANBI%in%sp.phylo)] # 86 spp. in SANBI records but not in phylo
...
summary(pgls(I(log(Rich_genus_SA)/sqrt(Area_genus))~I(log(median_height_genus)),data=compdatgenus3,lambda='ML'))


# plot genus level
names(stemAge_genus)=names(Rich_genus)
divdens=I(log(Rich_genus_SA)/sqrt(Area_genus)) #; divdens['Aristaloe']='NA'
par(mar=c(4,4,2,2))
...
abline(a=0.168,b=-0.0423,lty="dashed")
