...
coords=read.table('Coords_GBIF_cleaned_2020_01_07.txt',h=T,sep='\t')
head(coords)
dim(coords)
...
sp.GBIF[-which(sp.GBIF%in%sp.phylo)] # all species in GBIF records are in phylo ()
...
# plot Figure 3 : decoupling of diversification density and rate

#################################################################################################################################
############## PART IV.2.1: Plot diversity density vs diversification rate for many time slices and 100 trees ###################
#################################################################################################################################
par(mfrow=c(1,1))
...
points(as.numeric(divdens)~as.numeric(divrate),pch=4,col=1,cex=3)

c(minrate,maxrate,mindens,maxdens)

summary(CORS) 
sum(CORS<0)/length(na.exclude(CORS)) # 59.9% negative corr
