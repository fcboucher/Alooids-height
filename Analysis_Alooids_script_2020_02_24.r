...
# Georeferenced data from GBIF carefully curated using known distribution maps
...
plot(as.numeric(divdens)~as.numeric(divMOM),pch=4,xlab='Diversification rate (sp.Myr-1)',ylab='Diversification density (sp.km-1)',col=1,cex=0.02,log='xy',xlim=c(0.04,0.8),ylim=c(0.0002,0.1))
...
#text(y=as.numeric(divdens),x=as.numeric(divMOM),labels=c(1:3,NA,4:8,NA,9),cex=0.8,pos=3)
...
symbols(y=as.numeric(divdens),x=as.numeric(divMOM),circles=rep(1,11),fg=rgb(0,0,0,0.9))
